...
alpha_base <- function(
  ...
  if(isTRUE(phylo.div)){
    ...
    names(phy.dt)[(length(phy.dt)):length(phy.dt)] <- methods[length(methods)]  # This area may result in errors if additional measures added, untested
    tmp.dt <- tmp.dt[phy.dt] # adds the columns to the other dataframe with the previously calculated alpha scores
    
    names(tmp.dt)[2:ncol(tmp.dt)] <- methods
...
methods.alpha <- c("Shannon", "Simpson", # Non-phylogenetic measures, add additional measures here
                   "Phylogenetic", "Richness") %>%  # Phylogenetic measures
